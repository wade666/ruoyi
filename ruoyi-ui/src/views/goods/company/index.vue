    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['goods:company:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['goods:company:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listCompany, getCompany, delCompany, addCompany, updateCompany } from "@/api/goods/company";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        companyName: [
          { required: true, message: "仓库所属公司名不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除序号为"' + companyIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('goods/company/export', {
        ...this.queryParams
